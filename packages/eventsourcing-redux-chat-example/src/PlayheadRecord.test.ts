import { PlayheadError, RecordWithPlayhead } from './PlayheadRecord';
...
it('Can create custom playheadhead class', () => {
  ...
  const withCounts = state.increase().increase().get('counter');
  expect(withCounts).toBe(2);
...
it('Playhead should throw error when its not increased', () => {
  ...
  expect(() => state.setPlayhead(4)).toThrowError(PlayheadError.doesNotMatch(0, 4));
});
