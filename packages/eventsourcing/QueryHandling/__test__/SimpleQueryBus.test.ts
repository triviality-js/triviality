...
it('Should always be a handler for a Query', () => {
  ...
  const query = new TestQuery();
  return expect(simpleBus.dispatch(query)).rejects.toThrowError(QueryHandlerNotRegisteredError.missingHandler(query));
});
...
it('Cannot have 2 handlers for same Query', () => {
  ...
  expect(() => {
    simpleBus.subscribe(new TestQueryHandler());
  }).toThrowError('Function cannot be registered on TestQueryHandler.handle because already registered to other handler');
});
