...
it('Should always have event as argument', () => {
  ...
  }).toThrowError('Missing event argument on TestEventHandler.hasBoughtCar');
});
...
it('Should not have more then 2 arguments', () => {
  expect(() => {
    ...
    return new TestEventHandler();
  }).toThrowError('To many arguments for TestEventHandler.hasBoughtCar');
});
...
it('Should always give events when used as function', () => {
  ...
  expect(() => {
    ...
    return new TestEventHandler();
  }).toThrowError('Specify at least one event (HandleDomainEvent(<Event>)) or don\'t use it as function (TestEventHandler.hasBoughtCar)');
});
