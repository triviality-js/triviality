...
it('Should always be a handler for a command', () => {
  ...
  const command = new TestCommand();
  return expect(simpleBus.dispatch(command)).rejects.toThrow('Missing command handler registered for TestCommand');
});
...
it('Cannot have 2 handlers for same command', () => {
  ...
  expect(() => {
    simpleBus.subscribe(new TestCommandHandler());
  }).toThrowError('Function cannot be registered on TestCommandHandler.handle because already registered to other handler');
});
